...
def process_detection(data, filter_type='Car'):
    if isinstance(data, str):
        import io
        df = pd.read_csv(io.StringIO(data), header=None, sep=' ')
...
def process_point_cloud_with_3d_boxes(points, result_lines, calib_path='./cfgs/calib.txt'):
    ...
    if result_lines is None or result_lines == '' or result_lines == []:
        ...
        df = process_detection(result_lines)
    
    rect = calib.R0
    rect_ = np.zeros((4,4), dtype=float)
    rect_[0:3,0:3] = rect
    rect_[3,3] = 1
    
    
    Trv2c = calib.V2C
    Trv2c_ = np.zeros((4,4), dtype=float)
    Trv2c_[0:3,:] = Trv2c
    Trv2c_[3,3] = 1
    
    save_ = np.zeros((4,0), dtype=float)
    ...
    for o in range(len(df)):
        ...
        save_points1 = compute_3D_line(x1,x2, y1,y2, z1,z2,rgb)
        save_points2 = compute_3D_line(x2,x3, y2,y3, z2,z3,rgb)
        save_points3 = compute_3D_line(x3,x4, y3,y4, z3,z4,rgb)
        save_points4 = compute_3D_line(x4,x1, y4,y1, z4,z1,rgb)
        
        save_points = np.concatenate((save_points1,save_points2,save_points3,save_points4), axis=1)
        
        save_points11 = compute_3D_line(x11,x22, y11,y22, z11,z22,rgb)
        save_points22 = compute_3D_line(x22,x33, y22,y33, z22,z33,rgb)
        save_points33 = compute_3D_line(x33,x44, y33,y44, z33,z44,rgb)
        save_points44 = compute_3D_line(x44,x11, y44,y11, z44,z11,rgb)
        
        save_points_ = np.concatenate((save_points11,save_points22,save_points33,save_points44), axis=1)
        
        save_points111 = compute_3D_line(x1,x11, y1,y11, z1,z11,rgb)
        save_points222 = compute_3D_line(x2,x22, y2,y22, z2,z22,rgb)
        save_points333 = compute_3D_line(x3,x33, y3,y33, z3,z33,rgb)
        save_points444 = compute_3D_line(x4,x44, y4,y44, z4,z44,rgb)
        save_points__ = np.concatenate((save_points111,save_points222,save_points333,save_points444), axis=1)    
        
        save_ = np.concatenate((save_,save_points,save_points_, save_points__), axis=1)
    
    #pad = np.zeros((points.shape[0],3), dtype=float)
    save_points_velo = calib.project_rect_to_velo(save_[0:3,:].T)
